...
class NewCustomImageOrbitDataset(Dataset):
    def __init__(self, cfg: Any) -> None:
        super().__init__()
        self.cfg: CustomImageDataModuleConfig = parse_structured(CustomImageDataModuleConfig, cfg)
...
#         self.n_views = self.cfg.n_test_views
        ...
        camera_positions2: Float[Tensor, "B 3"] = torch.stack(
            [
                camera_distances * torch.sin(elevation),
                camera_distances * torch.cos(elevation) * torch.cos(azimuth),
                camera_distances * torch.cos(elevation) * torch.sin(azimuth),
        ...
        camera_positions[2] = camera_positions2[1]
        camera_positions[3] = camera_positions2[2]
#         # 将 camera_positions2 添加到 camera_positions 中
#         camera_positions = torch.cat([camera_positions, camera_positions2], dim=0)
        print(camera_positions)
