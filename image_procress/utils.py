...
def image_preprocess(input_image, save_path, lower_contrast=True, rescale=True):
    ...
    padded_mask = np.zeros((side_len, side_len), dtype=np.uint8)
    ...
    padded_mask = cv2.resize(padded_mask, (252, 252), interpolation=cv2.INTER_NEAREST)
    mask_save_path = save_path.replace('.png','_mask.png')
    cv2.imwrite(mask_save_path,padded_mask)
